...
- `/model` 显示的模型与导入时选择的模型一致。
- Nexly 控制台出现刚才的请求记录。
- CC Switch 中没有同时启用两个会互相覆盖的 Codex 供应商。
...
1. 完全退出 Codex。
2. 在 CC Switch 的 Codex 页面找到原供应商并点击 **启用**。
3. 重新打开终端并启动 Codex。
4. 使用 `/model` 和一条短消息确认已经恢复。
5. 如果 Nexly Key 不再使用，可在控制台将其禁用或删除。

切换供应商不会自动删除已经导入的 Nexly 配置。确认无需保留后，再从 CC Switch 中删除对应供应商。
