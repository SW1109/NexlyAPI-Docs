<!-- Canvas UI Ripple · https://canvasui.dev/docs/components/ripple
Copyright (c) 2026 David Haz. MIT + Commons Clause；完整许可见 LICENSE。
保留上游实现，官网 registry 获取于 2026-09-15。 -->
<script lang="ts">
export type RippleTrigger = "click" | "hover" | "none";

export interface RippleOptions {
  /** 本地适配：纯光纹覆盖层不捕获 HTML，避免依赖实验 API 或遮住底层 SVG。 */
...
const props = defineProps<RippleOptions>();
...
onMounted(async () => {
  native.value = props.captureContent !== false && supportsHtmlInCanvas();
  await nextTick();
  if (disposed) return;
